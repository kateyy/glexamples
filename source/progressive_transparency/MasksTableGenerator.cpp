    ...
    std::vector<mask_t> & combinationMasks)
{
    if (k == 0)
    {
        combinationMasks.push_back(static_cast<mask_t>(combination.to_ulong()));
        return;
    }
    ...
    maskDistribution_t & masks)
{
    ...
    const auto lowNumSamples = static_cast<int>(glm::floor(avgNumSamples));
    const auto highNumSamples = static_cast<int>(glm::ceil(lowNumSamples));
    ...
    maskDistribution_t::iterator & toMaskIt)
{
    ...
    {        
        if (numMasks >= fromMasks.size())
        {
            ...
            numMasks -= static_cast<unsigned int>(fromMasks.size());
        }
        else
        {
